...
use std::str;
use std;
use mdb::{MDB_env,MDB_txn,mdb_env_create,mdb_reader_check,mdb_env_set_maxdbs,mdb_env_set_mapsize,mdb_env_open,mdb_txn_begin,mdb_txn_commit,mdb_txn_abort,mdb_env_close,mdb_dbi_open,MDB_CREATE,MDB_DUPSORT,MDB_val,MDB_NOTFOUND};
use mdb;
...
pub fn open_repository(txn:*mut MDB_txn)->Result<Repository,c_int> {
...
pub fn open_base(t:*mut MDB_txn,base:&mut Result<c_uint,c_int>, name:&str, flags:c_uint)->Result<c_uint,c_int>{
    match *base {
        Ok(n)=>Ok(n),
        Err(_)=>{
            let mut x=0;
...
pub fn from_val(v:MDB_val)->Vec<u8>{unsafe {slice::from_raw_parts(v.mv_data,v.mv_size as usize).to_vec()}}
