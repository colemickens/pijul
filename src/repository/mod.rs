...
fn external_key(repo:&Repository,key:&[u8])->ExternalKey {
    unsafe {
        ...
        if key.len()>=HASH_SIZE && memcmp(key.as_ptr() as *const c_void,ROOT_KEY.as_ptr() as *const c_void,HASH_SIZE as size_t)==0 {
            ...
            match mdb::get(repo.mdb_txn,repo.dbi_external,&key[0..HASH_SIZE]) {
                Ok(pv)=> {
                ...
                Err(_)=>{
                    ...
                    //dump_table(repo.mdb_txn,repo.dbi_external);
...
pub fn record<'a>(repo:&'a mut Repository,working_copy:&std::path::Path)->Result<(Vec<Change>,HashMap<LocalKey,Inode>),Error>{
    ...
    dfs(repo,&mut actions, &mut line_num,&mut updatables,
...
fn internal_hash<'a>(repo:&Repository,key:&'a [u8])->Result<&'a [u8],c_int> {
    unsafe {
        if key.len()==HASH_SIZE && memcmp(key.as_ptr() as *const c_void,ROOT_KEY.as_ptr() as *const c_void,HASH_SIZE as size_t)==0 {
            Ok(ROOT_KEY)
        } else {
        ...
        match *ch {
            Change::Edges{ref edges} =>
                for e in edges {
                    ...
                    let _=unsafe {
                        let u=internal_hash(repo,&e.from[0..(e.from.len()-LINE_SIZE)]).unwrap();
                    ...
                    let _=unsafe {
                        let v=internal_hash(repo,&e.to[0..(e.to.len()-LINE_SIZE)]).unwrap();
                    ...
                    let _=unsafe {
                        ...
                        let p=internal_hash(repo,&e.introduced_by).unwrap();
                    ...
                    unsafe {
                        let _=mdb::del(repo.mdb_txn,repo.dbi_nodes,&pu[1..(1+KEY_SIZE)], Some(&pv));
                        let _=mdb::del(repo.mdb_txn,repo.dbi_nodes,&pv[1..(1+KEY_SIZE)], Some(&pu));
            ...
            Change::NewNodes { ref up_context,ref down_context,ref line_num,ref flag,ref nodes } => {
                ...
                for c in up_context {
                    let u= if c.len()>LINE_SIZE {
                        internal_hash(repo,&c[0..(c.len()-LINE_SIZE)]).unwrap()
                    } else {
                        internal_patch_id
                    };
...
pub fn register_hash(repo:&mut Repository,internal:&[u8],external:&[u8]){
    unsafe {
        ...
        mdb::put(repo.mdb_txn,repo.dbi_external,internal,external,0).unwrap();
        mdb::put(repo.mdb_txn,repo.dbi_internal,external,internal,0).unwrap();
    }
}
...
pub fn apply(repo:&mut Repository, patch:&patch::Patch, internal:&[u8])->Result<(),Error> {
    ...
    for ch in patch.changes.iter() {
        match *ch {
            Change::Edges{ref edges} =>{
                for e in edges {
                    let hu=internal_hash(repo,&e.from[0..(e.from.len()-LINE_SIZE)]).unwrap();
                    let hv=internal_hash(repo,&e.to[0..(e.to.len()-LINE_SIZE)]).unwrap();
                        ...
                        if e.flag&FOLDER_EDGE!=0 {
                            ...
                            if nonroot_is_alive(&mut cursor,pu) {
                                ...
                                while i<children.len() {
                                    connect_down(repo,pu,&children[i..(i+KEY_SIZE)],&internal);
                                    i+=KEY_SIZE
                                }
    ...
    for ref dep in patch.dependencies.iter() {
        let dep_internal=internal_hash(repo,&dep).unwrap();
        unsafe {
...
pub fn write_changes_file(repo:&Repository,changes_file:&Path)->Result<(),Error> {
...
pub fn dependencies(changes:&[Change])->Vec<patch::ExternalHash> {
    ...
    fn push_dep(deps:&mut Vec<patch::ExternalHash>,dep:patch::ExternalHash) {
...
fn filename_of_inode<'a>(repo:&'a Repository,inode:&[u8],working_copy:&mut PathBuf)->bool {
...
fn unsafe_output_repository(repo:&mut Repository, working_copy:&Path) -> Result<Vec<(Vec<u8>,Vec<u8>)>,Error>{
                            ...
                            path.push(std::str::from_utf8(filename).unwrap());
                            {
                                ...
                                *nfiles+=1;
                                vec.push((c[1..(1+KEY_SIZE)].to_vec(),parent_inode.to_vec(),inode.to_vec(),
                                          if filename_of_inode(repo,inode,&mut buf) {Some(buf)} else { None },
                                          perms))
                            }
    ...
    let mut buf=PathBuf::from(working_copy);
    let mut nfiles=0;
    retrieve_paths(repo,working_copy,&ROOT_KEY,&mut buf,ROOT_INODE,&mut paths,&mut cache,&mut nfiles);
...
fn update_tree(repo:&mut Repository,updates:Vec<(Vec<u8>,Vec<u8>)>){
    for (par,inode) in updates {
        unsafe {
            mdb::put(repo.mdb_txn,repo.dbi_tree,&par,&inode,0);
            mdb::put(repo.mdb_txn,repo.dbi_revtree,&inode,&par,0);
        }
    }
}
...
pub fn output_repository(repo:&mut Repository, working_copy:&Path, pending:&patch::Patch) -> Result<(),Error>{
    unsafe {
        ...
        if e==0 {
            ...
            apply(repo,pending,&internal[..]);
            let updates=try!(unsafe_output_repository(repo,working_copy));
            mdb_txn_abort(txn);
            repo.mdb_txn=parent_txn;
            update_tree(repo,updates);
            Ok(())
        } else {
